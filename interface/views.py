...
class DueDecksHTMLView(APIView):
    def get(self, request):
        ...
        return render(request, 'decks.html', {'due_decks_data': due_decks_context})
...
def deck_session_view(request, deck_id):
    ...
    if session_deck_id != deck_id or (session_deck_id == deck_id and not session_card_ids) or not session_deck_id:
...
def update_card_view(request, card_pk): 
    ...
    if current_card_index is None or not session_card_ids or \
       ...
       session_card_ids[current_card_index] != card.id:
        logger.warning(f"Card update attempt with inconsistent session for user {request.user.id}, card {card_pk}.")
    ...
    if next_card_index >= len(session_card_ids):
        _clear_flashcard_session(request.session)
        return render(request, 'partials/session_finished_partial.html', {'deck': deck})
    ...
    return render(request, 'partials/card_container_partial.html', {'card': next_card, 'deck': deck})
